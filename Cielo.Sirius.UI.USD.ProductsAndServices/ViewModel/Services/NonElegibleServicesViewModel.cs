...
{
    class NonElegibleServicesViewModel : ViewModelBase
    {
        #region Constants
        ...
        private Logger _logger = Logger.LoggerFor<NonElegibleServicesViewModel>();
                ...
                if (_serviceSelectedCommand == null)
                {
                    _serviceSelectedCommand = new RelayCommand(
                        param =>
                        {
                            ...
                            Navegate("NonElegibleServiceDetails", "ProductsAndServicesMainRegion", "", paramsDic);
                        },
                        param => true
                    );
                }
        ...
        private void LoadServicesCommandMethod()
        {
            ...
            var servicesModel = new NonElegibleServicesModel();
            servicesModel.Request = new ConsultarServicoNaoElegivelClienteRequest();
            servicesModel.Request.CodigoCliente = clientIdNumber;

            servicesModel.Execute();
            var response = servicesModel.Response;

            if (response != null && (response.Status.Equals(ExecutionStatus.Success) || response.Status.Equals(ExecutionStatus.Warning)))
            {
        ...
        private void FilterServicesMethod(object param_)
        {
            ...
            else
            {
                ...
                {
                    // Check if exists any text that does not exist in the itens 
        ...
        public class Service
        {
            ...
            public ConsultarServicoNaoElegivelClienteServicoDTO ServiceDTO { get; set; }
        }
